...
- api/qa/list	查询QA问答的表单。GET方法，参数仅一个page表示查询第几页（默认为1），返回值：
...
- api/qa/release 发布。POST方法，必须管理员账号登陆后访问
...
- api/qa/delete 和删除帖子的api一切相同，必须管理员账号登陆后访问。

- api/qa/chtop 改变置顶状态。POST方法，参数仅id一个值，返回值success或者fail。必须管理员账号登陆后访问。

- qa/<%d> 查看某条帖子。GET方法，无参数，返回查看帖子的HTML页面。
