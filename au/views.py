...
class Chavatar(ViewBase):
    ...
    def post(self, request):
        ...
        if file_suffix not in self.PICTURE_SUFFIX:
            return self.fail('图片格式不对呢')
        ...
        user = request.user
        if not isinstance(user, UstcUser):
            return self.fail('出现系统内部错误，可能出bug了？')
        user.avatar.save(file_name, file_content)
        user.save()
        #UstcUser.objects.filter(pk=request.user.id).update(avatar=file)
        return JsonResponse({  "result" : "success", "avatar" :  file_name })